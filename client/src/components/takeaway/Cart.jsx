...
// ============================================================
// RECEIPT PRINTER (With Dynamic Global Configurations)
// ============================================================
const printReceipt = ({ cartItems, subtotal, tableName, waiterName, orderNumber, config }) => {
  ...
  // Fallbacks to handle fresh initialization periods gracefully
  ...
  const kotRows = cartItems.map(item => `
    <tr>
      ...
      <td class="ki-qty">× ${item.quantity}</td>
    </tr>
  `).join('');
...
<body>
  ...
  <div class="receipt">
    ...
    <div class="meta-grid">
      ...
      <div class="meta-item">
        ...
        <span class="meta-value">${orderNum}</span>
      </div>
      <div class="meta-item">
      ...
      <div class="meta-full">
        ...
        <span class="meta-value">${waiterName}</span>
      </div>` : ''}
    </div>
    ...
    <div class="totals">
      ...
      <div class="totals-row">
        <span class="label">GST (${taxPercent}%)</span>
        <span class="value">Rs. ${taxAmount.toLocaleString()}</span>
      </div>
      <div class="totals-row">
        <span class="label">Service Charges (${servicePercent}%)</span>
        <span class="value">Rs. ${serviceAmount.toLocaleString()}</span>
      </div>
  ...
  <div class="receipt kot kot-section">
    <div class="kot-header">
      <div class="kot-title">
        KITCHEN ORDER
        <span class="kot-sub">— K O T —</span>
      </div>
    </div>

    <div class="kot-meta">
      <div class="kot-meta-row">
        <span class="km-label">Order #</span>
        <span class="km-value">${orderNum}</span>
      </div>
      <div class="kot-meta-row">
        <span class="km-label">Type</span>
        <span class="km-value">${orderType}</span>
      </div>
      ${waiterName ? `
      <div class="kot-meta-row">
        <span class="km-label">Waiter</span>
        <span class="km-value">${waiterName}</span>
      </div>` : ''}
      <div class="kot-meta-row">
        <span class="km-label">Time</span>
        <span class="km-value" style="font-size:12px;">${timeStr}</span>
      </div>
    </div>
    ...
    <table class="kot-items">
      <thead>
        <tr>
          <th class="ki-item">Item</th>
          <th class="ki-qty">Qty</th>
        </tr>
      </thead>
      <tbody>${kotRows}</tbody>
    </table>
...
const Cart = ({
  ...
  // Config state to dynamically load Global settings parameters
  ...
  const fetchSystemVariables = async () => {
    try {
      ...
      console.error('Cart failed to synchronize with global settings router:', err);
    }
  };

  // Base Calculation Subtotals
  const subtotal = cartItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
  
  // Rate parsing transformations
  const taxPercent = parseFloat(config.tax_rate || 0);
  const servicePercent = parseFloat(config.service_charges || 0);

  // Core Financial Additions
  const taxAmount = Math.round(subtotal * (taxPercent / 100));
  const serviceAmount = Math.round(subtotal * (servicePercent / 100));
  const grandTotal = subtotal + taxAmount + serviceAmount;
      ...
      {cartItems.length > 0 && (
        <div className="p-6 border-t border-slate-100 bg-slate-50/50">
          <div className="space-y-3 mb-6">
            ...
            {/* Dynamic Service Charges Output */}
            <div className="flex justify-between text-sm text-slate-500 font-medium">
          ...
          <button
            ...
            disabled={isPlacing}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary-500 py-4 text-base font-bold text-white shadow-lg shadow-primary-500/25 hover:bg-primary-600 hover:-translate-y-0.5 active:translate-y-0 active:shadow-md transition-all duration-150 disabled:opacity-50"
          >
            {isPlacing ? (
